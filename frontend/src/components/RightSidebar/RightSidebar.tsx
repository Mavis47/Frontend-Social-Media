...
export default function RightSidebar() {
    ...
    const SendFollowRequest = async(followingId: number) => {
        ...
        if (!auth.token) return;
        const followData = await axios.post(`http://localhost:5001/api/follow/userToFollow`,{followerId,followingId},{
            headers: {
                Authorization: `Bearer ${auth.token}`
            }
        })
  ...
  return (
    <div className='rightsidebar'>
        <div className="title">
            <div className="suggested"><p>Suggested For You</p></div>
            <div className="SeeAll">See All</div>
        </div>
        <div className="follow">
            {userdata?.map((profile) => (
                <div className="Follow-content" key={profile.id}>
                <img src={profile.userProfilePic} alt="" className="profile-photo"/>
                    <div className="profile-follow">
                        
                        <span>{profile.username}</span>
                        <span>Suggested For You</span>
                    </div>
                    <button className='text-blue-500 follow-btn' onClick={() => SendFollowRequest(profile.id)}>{followStatus[profile.id] ? "Sent" : "Follow"}</button>
                </div>
            ))}
        </div>
    </div>
