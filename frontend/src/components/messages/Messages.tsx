...
export default function Messages() {
  ...
  const getloggedinUser = async() => {
    ...
    const loggedInUser = await axios.get(`http://localhost:5001/api/auth/getMe`,{
      headers: {
    ...
    const showUsers = async() => {
        ...
        const Usersdata = await axios.get(`http://localhost:5001/api/auth/allUsers`,{
            headers: {
                Authorization: `Bearer ${auth.token}`
            }
        });
    ...
    const fetchMessages = async (receiverId: number) => {
      ...
      const messagesData = await axios.get(
        `http://localhost:5001/api/message/getMessage/${user.id}/${receiverId}`);
    ...
    const sendMessage = async() => {
      ...
      const response = await axios.post(`http://localhost:5001/api/message/sendMessage`,{
        senderId: auth.user,
    ...
    const handleDelete = async(messageId: number) => {
       const deletedata = await axios.delete(`http://localhost:5001/api/message/deleteMessage/${messageId}`)
    ...
    <div className="message-container">
      ...
      <div className="messageContainer">
        <div className="container">
          <img src={user.userProfilePic} alt="" className="profile-picture ml-4" />
        ...
        <div className="messages">
          <div className="message-others">
            {messages
                ...
                .map((message) => (
                  <p key={message.id} id="paragraph-others">{message.content}<span onClick={() => handleDelete(message.id)}><DeleteIcon className="deleteIcon"/></span></p>
                  ))}
          </div>
          ...
          <div className="message-self">
            {messages
                ...
                .map((message) => (
                <p id="paragraph-self" key={message.id}>{message.content}<br/><span onClick={() => handleDelete(message.id)}><DeleteIcon className="deleteIcon"/></span></p> 
